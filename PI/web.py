...
class CameraService:
    ...
    def __init__(self, auto_start=False):  # Cambiado a False por defecto
        ...
        self.height = 480
        self.fps = 30
        self.auto_start = auto_start  # Nueva propiedad
        
    def add_client(self, client_id):
        self.clients.add(client_id)
        logger.info(f"[SOCKET.IO] Cliente {client_id} conectado. Total: {len(self.clients)}")
        socketio.emit('connection_status', {'status': 'connected'}, room=client_id)
        
        # Iniciar automáticamente la transmisión solo si auto_start está activado
        if self.auto_start and not self.stream_active and len(self.clients) > 0:
            self.start_stream()
...
def handle_connect():
    ...
    logger.info(f"[SOCKET.IO] Nuevo cliente conectado: {client_id}")
    ...
    logger.info(f"[SOCKET.IO] Enviado estado de motores a cliente {client_id}")
    
    # Enviar estado actual de servos al cliente que se conecta
    socketio.emit('servo_status', {'status': motor_service.servo_status}, room=client_id)
    logger.info(f"[SOCKET.IO] Enviado estado de servos a cliente {client_id}")

# handle_control_servos para manejar comandos de calibración
...
def handle_control_servos(data):
    ...
    # Simplificar comandos para encendido/apagado básico
    if action == "move":
        # Encender y mover servo
        if 'angle' not in data:
            return {'success': False, 'response': 'Ángulo no especificado'}
        
        angle = int(data['angle'])
        command = f"servo,{servo_type},move,{angle}"
        
        # Enviar comando al Arduino
        motor_service.servo_arduino.write(f"{command}\n".encode())
        logger.info(f"Comando enviado a Arduino: {command}")
        
        return {'success': True, 'response': f"Comando enviado: {command}"}
    
    elif action == "stop":
        # Detener servo (simplificado a "off")
        command = f"servo,{servo_type},off"
        
        # Enviar comando al Arduino
        motor_service.servo_arduino.write(f"{command}\n".encode())
        logger.info(f"Comando enviado a Arduino: {command}")
        
        return {'success': True, 'response': f"Servo {servo_type} detenido"}
...
def main():
    try:
        ...
        # Elimina el parámetro allow_unsafe_werkzeug
