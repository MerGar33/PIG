...
export function initMotorControl() {
    try {
        ...
        motorSocket = io(CONFIG.MOTOR.WS_URL, {
            transports: ['websocket', 'polling'],  // Priorizar websocket sobre polling
            reconnection: true,
            reconnectionAttempts: 5,              // Reducir intentos
            reconnectionDelay: 500,               // Reducir delay
        ...
        motorSocket.on('connecting', () => {
            console.log('Conectando a socket...');
            window.logMessage('Conectando a socket...');
        });

        motorSocket.on('reconnect_attempt', (attemptNumber) => {
            console.log(`Intento de reconexión #${attemptNumber}`);
            window.logMessage(`Intento de reconexión #${attemptNumber}`);
        });
        
        motorSocket.on('connect', () => {
            console.log('Conectado al control con ID:', motorSocket.id);
            window.logMessage('Conectado al servidor. Inicializando dispositivos...');
        ...
        motorSocket.on('connect_error', (error) => {
            ...
            window.logMessage("Error de conexión: " + error.message, true);
        });
        
        motorSocket.on('connect_timeout', () => {
            console.error("Timeout en la conexión Socket.IO");
            window.logMessage("Timeout en la conexión", true);
        });
        
        // Configurar eventos para recibir estados
        ...
        motorSocket.on('arduino_status', (status) => {
            ...
            window.logMessage(`Arduino de motores: ${motorsConnected ? 'Conectado' : 'Desconectado'}`);
            window.logMessage(`Arduino de servos: ${servosConnected ? 'Conectado' : 'Desconectado'}`);
        });
        
        motorSocket.on('disconnect', () => {
            console.error("Desconectado del servidor");
            window.logMessage("Desconectado del servidor", true);
        ...
        window.logMessage("Error al inicializar control: " + error, true);
    }
}
